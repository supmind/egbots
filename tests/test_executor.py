...
class TestRuleExecutor(AsyncTestCase):
    ...
    def test_simple_action_execution(self, mock_action_reply):
        """测试一个简单的动作是否被正确调用。"""
        script = "WHEN message THEN reply('hello there')"
    ...
    def test_condition_evaluation_true(self, mock_action_send_message):
        """测试当 IF 条件为真时，动作被执行。"""
        script = "WHEN message IF user.first_name == 'Test' THEN send_message('Condition met') END"
        parsed_rule = RuleParser(script).parse()
        executor = RuleExecutor(self.update, self.context, self.db_session)
    ...
    def test_condition_evaluation_false(self, mock_action_send_message):
        """测试当 IF 条件为假时，动作不被执行。"""
        script = "WHEN message IF user.first_name == 'WrongName' THEN send_message('Should not be called') END"
        parsed_rule = RuleParser(script).parse()
        executor = RuleExecutor(self.update, self.context, self.db_session)
    ...
    def test_full_rule_execution_selects_correct_block(self, mock_action_reply):
        ...
        script = """
        WHEN message
        IF user.first_name == "wrong" THEN reply("if")
        ELSE IF user.id == 123 THEN reply("else if")
        ELSE THEN reply("else")
        END
        """
    ...
    def test_set_var_action(self, mock_action_set_var):
        ...
        script = "WHEN message THEN set_var('user.warnings', '1')"
        parsed_rule = RuleParser(script).parse()
    ...
    def test_resolve_path_for_db_variable(self):
        ...
        self.db_session.query.return_value.filter_by.return_value.filter.return_value.first.return_value = mock_var
