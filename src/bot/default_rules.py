...
WHEN command WHERE command.name == 'unmute' AND user.is_admin == true THEN {
    ...
    if (target_id != null) {
        ...
        reply("<b>使用方法:</b><br>- 回复消息: <code>/unmute</code><br>- 使用ID: <code>/unmute &lt;user_id&gt;</code>", "HTML");
    }
}
END
"""
...
WHEN command WHERE command.name == 'mute' AND user.is_admin == true THEN {
    ...
    if (target_id != null and duration != null) {
        ...
        if (reason != "") {
            reply_text = reply_text + "<br><b>原因:</b> " + reason;
        }
        reply(reply_text, "HTML");
        log("用户 " + target_id + " 被 " + user.id + " 禁言 " + duration + "。原因: " + (reason or "未提供"), "moderation");
    } else {
        reply("<b>使用方法:</b><br>- 回复消息: <code>/mute &lt;时长&gt; [原因]</code><br>- 使用ID: <code>/mute &lt;user_id&gt; &lt;时长&gt; [原因]</code>", "HTML");
    }
}
END
"""
    ...
    },
    {
        ...
        "script": """WHEN command WHERE command.name == "id" THEN { text = "你的用户ID: <code>" + user.id + "</code><br>" + "当前群组ID: <code>" + message.chat.id + "</code>"; if (message.reply_to_message) { text = "被回复用户ID: <code>" + message.reply_to_message.from_user.id + "</code><br>" + "你的用户ID: <code>" + user.id + "</code><br>" + "当前群组ID: <code>" + message.chat.id + "</code>"; } reply(text, "HTML"); } END"""
    },
    {
        "name": "[信息] 帮助命令",
        "priority": 200,
        "description": "响应 /help 命令，提供一段默认的帮助文本。",
        "script": """WHEN command WHERE command.name == "help" THEN { help_text = "本群由一个强大的规则引擎机器人驱动。<br>" + "管理员可以自定义规则来实现自动化管理。<br>" + "目前可用的公开命令: <code>/id</code>, <code>/help</code>"; reply(help_text, "HTML"); } END"""
    },
    {
        "name": "[功能] 新成员欢迎",
        "priority": 990,
...
WHEN command WHERE command.name == 'ban' AND user.is_admin == true THEN {
    ...
    if (target_id != null) {
        ...
        reply("<b>使用方法:</b><br>- 回复用户消息: <code>/ban [原因]</code><br>- 使用ID: <code>/ban &lt;user_id&gt; [原因]</code>", "HTML");
    }
}
END
"""
